...
const API_BASE_URL = 'https://api-reportediario.cne.gob.mx/api/EstacionServicio/Petroliferos';
const OUTPUT_FILE = path.join(__dirname, 'gas-prices.json');
...
let mexicanStates = [
    ...
    {
        state: "Baja California Sur", county: [
            "Comondú",
            "La Paz",
            "Loreto",
            "Los Cabos",
    ...
    {
        state: "Coahuila de Zaragoza", county: [
            "Abasolo",
            "Acuña",
            "Allende",
            "Arteaga",
    ...
    {
        state: "Michoacán de Ocampo", county: [[
            "Acuitzio",
            "Aguililla",
            "Álvaro Obregón",
            "Angamacutiro",
    ...
    {
        state: "Veracruz de Ignacio de la Llave", county: [
            "Acajete",
            "Acatlán",
            "Acayucan",
            "Actopan",
...
function getMunicipalityCombos() {
    return mexicanStates.flatMap((stateData, stateIndex) => {
        const entidadId = padId(stateIndex + 1, 2);

        return stateData.county.map((municipality, municipalityIndex) => ({
            state: stateData.state,
            municipality,
            entidadId,
            municipioId: padId(municipalityIndex + 1, 3)
        }));
    });
}

function buildPetroliferosUrl({ entidadId, municipioId }) {
...
async function fetchPetroliferos(combo) {
    const url = buildPetroliferosUrl(combo);
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
        try {
            const response = await fetch(url, {
                headers: { 'User-Agent': 'gas-prices-api/1.0' },
            ...
            if (response.status < 500 && response.status !== 429) {
                throw new Error(`Petroliferos request failed for ${combo.entidadId}/${combo.municipioId} with status ${response.status}`);
            }
            ...
            if (attempt === MAX_RETRIES || error.message.includes('with status')) {
                throw error;
            }
        }

        if (attempt < MAX_RETRIES) {
            logMessage(`Retrying ${combo.state} / ${combo.municipality} (${attempt + 1}/${MAX_RETRIES}) after a temporary upstream response`);
        }
...
function cleanGasPrices(data) {
    return data.results.flatMap(summarizeResult);
}

async function scrapeGasPrices({ limit, concurrency = DEFAULT_CONCURRENCY } = {}) {
    const combos = getMunicipalityCombos();
    const selectedCombos = Number.isInteger(limit) ? combos.slice(0, limit) : combos;
    const results = new Array(selectedCombos.length);
    const workerCount = Math.min(Math.max(1, concurrency), selectedCombos.length || 1);
    let nextIndex = 0;
    let completed = 0;
    const startedAt = Date.now();
    let lastProgress = 0;

    logMessage(`Scrape started: ${selectedCombos.length} requests, ${workerCount} workers, ${REQUEST_TIMEOUT_MS / 1000}s timeout`);

    async function worker() {
        while (nextIndex < selectedCombos.length) {
            const index = nextIndex++;
            results[index] = await fetchPetroliferos(selectedCombos[index]);
            completed += 1;

            if (completed === selectedCombos.length || completed - lastProgress >= PROGRESS_INTERVAL) {
                lastProgress = completed;
                const elapsedSeconds = (Date.now() - startedAt) / 1000;
                const rate = completed / Math.max(elapsedSeconds, 0.001);
                const remainingSeconds = (selectedCombos.length - completed) / Math.max(rate, 0.001);
                logMessage(`Progress: ${completed}/${selectedCombos.length} (${Math.round(completed / selectedCombos.length * 100)}%) at ${rate.toFixed(1)} req/s, ETA ${Math.ceil(remainingSeconds)}s`);
            }
        }
    }
...
if (require.main === module) {
    ...
    scrapeAndSave({ limit: firstOnly ? 1 : undefined })
        .then(({ outputFile, data }) => {
            console.log(`Saved ${data.length} product summaries to ${outputFile}`);
        })
        .catch((error) => {
            console.error(error);
            process.exitCode = 1;
